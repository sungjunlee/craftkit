...
- `spec-charter` create/amend proposes one conditional pointer line (`Read spec/charter.md before proposing scope, direction, or Non-Goal changes.`) in `AGENTS.md`, plus a `@AGENTS.md` import in `CLAUDE.md` when neither file already imports or symlinks the other, in the same confirm as the charter write; writes only after confirm. No marker-bounded cache, no `revision=` stamping, and reassess no longer runs a drift check against it (#178, #249).
...
- Spec-axis topology (monorepo vs sibling-git vs type-1) and harness-pointer authority live in `skills/spec-charter/references/spec-axis.md` instead of being restated per skill (#177).
...
- `spec-charter` retires an Objective by deleting its line and listing the ID under `Retired IDs (never reuse)` (git keeps the text; no `docs/spec-history.md`) (#251). `spec-charter` and `spec-grill` Decisions tables become standing decisions (`date | decision | rationale`, no `supersedes` column): a flipped decision is rewritten in place with one clause on why the previous position was left, and a row is removed once its rejection reason no longer holds, human-gated same as Tier 1/Goal-Scope. `spec-charter amend` gains a `reset` path for concept changes: bump `revision`, rewrite Tier 1-3 freely, leave `Previous charter: git <sha>` under Decisions (#247).
- `spec-grill` `## Learnings` drops the `LEARN:BEGIN`/`LEARN:END` marker gate and the bounded-writer/Learning-Action requirement; any agent or human may add, update, or delete an entry directly under a content rule (one lesson with its why, no duplicates, delete when wrong), and only promotion to a standing Decision or the charter stays human-gated. `spec-charter` reassess drops Learning Action as a mutation family — Learnings stay a sensor, and promotion routes through the existing Grill/Amend Candidate families (#248).
- `spec-charter` and `spec-grill` move their Verification prompts to `references/verification.md`; the section is optional in the spec-* family contract (#252).
