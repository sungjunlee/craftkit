...
- `craft-autoresearch` stops when the run's outcome criteria are met: the forced "strengthen the suite before mutating" gate at a near-ceiling baseline is gone, and a high baseline may simply mean no change is needed. Suite strengthening now needs evidence from real outputs, and freezes the criteria — and the eval runner — before rebaselining. Dropped the quotas that had no basis: the mandatory eval/output counts, the 50% deterministic quota, the every-5th-experiment deletion, the mandatory one-week re-run, the 200%-size bloat trigger, the three-DISCARD escalation threshold, and the two pre-baseline contract fields (4th diagnostic, first-mutation hypothesis preview). Recommended ranges that are labelled as recommendations — suite sizing, holdout ≥2, the `skill_lines` bands — stay. Checkpointing now happens before the edit rather than after it; a rejected holdout is handled physically (restore the session's mutable files from the baseline checkpoint, leave unrelated edits alone, log the rejection) rather than only reported; and a KEEP is named as a provisional candidate until the holdout gate accepts it. Format checks may lead a KEEP when a real consumer requires the format, while stylistic proxies stay floor checks; action-safety evals score what the caller authorized and which gates the task requires, not the presence of a confirmation prompt; and reading holdout outputs to diagnose a further pass is called what it is — the holdout is spent, so a later acceptance needs fresh inputs or an explicit disclosure. Runner design now records executor, model settings, and loaded instructions/tools alongside the command, with an estimate-and-say-so rule when cost isn't knowable. Bounded budget, baseline-before-mutation, outcome-led KEEP/DISCARD, holdout isolation during the loop or an explicit limited-evidence waiver, and exact mutable-file rollback are unchanged. Prior-run observations are kept as clearly-marked non-normative history. The spine `## Example` is now a short no-change-at-a-valid-baseline session instead of a second full log; `references/worked-example.md` is the one end-to-end session (absorbing the deleted `references/contract-example.md`), with corrected arithmetic, an external run directory for every input, output, and checkpoint, and no history-rewriting rollback commands.
- `craft-critique`'s `references/failure-modes.md` drops the fixed review template and the top-three cap that contradicted the spine's judgment contract, and reads as symptom → candidate repair conditional on evidence and on what the artifact's consumers and callers actually require. The blanket "XML travels everywhere" portability claim is gone. The spine adds a step for reading what the artifact operates inside — its reference files, the instructions loaded with it, and its caller/consumer contracts — rather than judging a lone prompt, asks each finding to say whether it rests on an observed defect, a contradiction established by inspection (which needs no production failure first), or a predicted effect, and names "nothing here needs changing" as a valid result. The read-only default and the edit-on-request boundary are unchanged.
