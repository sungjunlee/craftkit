...
- `## Decisions` in `spec/charter.md` and `spec/capabilities.md` holds standing decisions, not a history ledger: a flipped decision is rewritten in place (rationale keeps one clause on why the previous position was left), and a row is removed once its rejection reason no longer holds. Git is the archive.
