...
Each example pairs a predicate with the concrete check that advances its status. A predicate whose check is only producer-observable caps out below the top status — write predicates a consumer can observe. The gate is in [`amendment.md`](amendment.md).
