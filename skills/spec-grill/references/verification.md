...
- "A commit scope appears often but has no docs, tests, or distinct behavior." Expected: keep it as a candidate seed or merge it into a supported capability.
...
- "문서 적을 건 적고 다음 제안해줘." Expected: if the file is absent and no keep condition holds, stop without a create proposal; otherwise treat it as edit intent, name the supported next capability, and ask before writing unless the authorization is explicit.
