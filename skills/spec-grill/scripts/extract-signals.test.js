...
`);
  ...
  it("collects folded YAML skill descriptions without leaking the block marker", () => {
    write(repo, "skills/craft-handoff/SKILL.md", `---
name: craft-handoff
description: >-
  End a long session with a durable handoff and resume prompt.
  Use when context should survive session boundaries.
---
`);

    assert.equal(
      readFrontmatterValue(
        fs.readFileSync(path.join(repo, "skills/craft-handoff/SKILL.md"), "utf8"),
        "description",
      ),
      "End a long session with a durable handoff and resume prompt. Use when context should survive session boundaries.",
    );

    const [candidate] = collectSkillCandidates(repo);
    assert.equal(candidate.name, "craft-handoff");
    assert.match(candidate.signal, /End a long session/);
    assert.doesNotMatch(candidate.signal, />-/);
  });
...
describe("summarizeEvidence", () => {
  it("marks candidates with two evidence classes as supported", () => {
    const summary = summarizeEvidence({
      system_map: ["system-map:sync"],
      readme: [],
      skill: [],
      scripts: ["script:sync"],
      docs: [],
    ...
    assert.deepEqual(summary.evidence_classes, ["system_map", "scripts"]);
    assert.equal(summary.admission_hint, "supported");
    assert.match(summary.admission_reason, /Supported by 2 evidence classes/);
  });

  it("keeps directory-only candidates as weak single-source seeds", () => {
    const summary = summarizeEvidence({
      system_map: [],
      readme: [],
      skill: [],
    ...
    assert.equal(summary.evidence_class_count, 1);
    assert.deepEqual(summary.evidence_classes, ["source_dirs"]);
...
`);
    ...
    assert.equal(ingest.evidence_class_count, 2);
    assert.equal(ingest.admission_hint, "supported");
  ...
  it("brownfield commit-scope-only: keeps provenance explicit without fake ownership", () => {
    ...
    assert.equal(progressSync.evidence_class_count, 1);
    assert.equal(progressSync.admission_hint, "weak-single-source");
...
`);
    ...
    assert.ok(candidate.evidence_class_count >= 4);
    assert.equal(candidate.admission_hint, "supported");
...
describe("formatHumanReport", () => {
  ...
  it("renders raw capability signals under the summary limit", () => {
    ...
    assert.match(report, /admission: interview-seed/);
  });
});
